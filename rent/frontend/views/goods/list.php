...
$form = ActiveForm::begin(['action'=>'index.php?r=goods/addgoods','method'=>'post','options' => ['enctype' => 'multipart/form-data']]) ?>
                                ...
                                <?php foreach($data as $key=>$val){?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href=""><?php echo $val['goods_id'];?></a>
                                        </td>
                                        <td><?php echo $val['goods_name'];?></td>
                                        <td class="hidden-480"><?php echo $val['goods_sort'];?></td>
                                        <td><?php echo $val['goods_price'];?></td>

                                        <td class="hidden-480">
                                            <span class="label label-sm label-warning"><?php echo $val['goods_num'];?></span>
                                        </td>
                                        <td id="state_<?php echo $val['goods_id'];?>">

                                            <?php
                                            if($val['goods_state']==0){
                                                echo "<a href='javascript:void(0);' onclick='change_state(this);' id='".$val['goods_id']."'>审核未通过</a>";
                                            }else{
                                                echo "<a href='javascript:void(0);' onclick='change_state(this);' id='".$val['goods_id']."'>审核通过</a>";;
                                            }
                                            //echo $val['goods_state'];?></td>
...
<script type="text/javascript">
    ...
    function change_state(obj){
        ...
        $.ajax({
            type: "POST",
            url: "index.php?r=goods/gaistate",
            data: "goods_id="+goods_id,
            success: function(msg){
                if(msg==1){
                    //审核通过的
                    $("#state_"+goods_id).html("<a href='javascript:void(0);' onclick='change_state(this);' id='"+goods_id+"'>审核通过</a>");
                }else if(msg==0){
                    $("#state_"+goods_id).html("<a href='javascript:void(0);' onclick='change_state(this);' id='"+goods_id+"'>审核未通过</a>");
                }else{
                    alert("error exits");
                }
            }
